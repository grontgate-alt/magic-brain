...
class RAGStore:
    ...
    def search(self, query_vec: list[float], limit: int = 5, privacy_filter: str = None):
        ...
        if privacy_filter:
            query_filter = Filter(
                must=[FieldCondition(key="privacy", match=MatchValue(value=privacy_filter))]
            )

        results = self.client.query_points(
            collection_name=self.collection, query=query_vec, limit=limit, query_filter=query_filter
        )
