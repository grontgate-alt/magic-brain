...
async def post(ep: str, data: dict, t: float = TIMEOUT) -> dict:
    async with httpx.AsyncClient(timeout=t) as c:
        ...
        return {"code": r.status_code, "data": r.json() if "application/json" in r.headers.get("content-type","") else r.text, "time": r.elapsed.total_seconds()}
...
async def test_agent_file_ops():
    ...
    for q, exp in tests:
        ...
        tag = r["data"].get("tag","?") if isinstance(r["data"],dict) else "?"
        is_agent = "[🛠️" in tag or "agent" in str(r["data"].get("model_used","") if isinstance(r["data"],dict) else "")
        ok = r["code"]==200 and is_agent
        T.log(ok, f"Agent [{exp}]", f"{tag} | {r['time']*1000:.0f}ms")
        results.append(ok)
    try: os.remove(os.path.expanduser("~/mb_test.txt"))
    except: pass
    return all(results)
...
async def test_edge_cases():
    cases = [("", "пустой"), ("!"*3000, "длинный"), ("{[<@#$%^&*()]}>", "спецсимволы"), ("привет "*200, "повторы")]
    results = []
    for q, desc in cases:
...
async def test_models_endpoint():
    ...
    d = r.json() if ok else {}
    T.log(ok, "Models endpoint", f"local:{len(d.get('local',[]))}, cloud:{len(d.get('cloud',[]))}")
    return ok
...
async def test_error_handling():
    async with httpx.AsyncClient(timeout=10) as c:
        r = await c.post(f"{BASE}/process", content="not json", headers={"Content-Type":"application/json"})
...
async def run_all():
    ...
    tests = [
        ("Health", test_health), ("Basic chat", test_basic_chat), ("Force modes", test_force_modes),
        ("Agent file ops", test_agent_file_ops), ("Privacy routing", test_privacy_routing),
        ("Edge cases", test_edge_cases), ("OpenAI endpoint", test_openai_endpoint),
        ("Models endpoint", test_models_endpoint), ("Performance", test_performance),
    ...
    if passed == len(results): print(f"{T.G}🎉 Все тесты пройдены! Система готова к продакшену.{T.E}")
    elif passed >= len(results)*0.8: print(f"{T.Y}⚠️ Большинство тестов ОК. Проверь упавшие.{T.E}")
    else: print(f"{T.R}❌ Много неудач. Требуется диагностика.{T.E}")
    print(f"{T.B}{'='*50}{T.E}\n")
