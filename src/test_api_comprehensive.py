...
def log(msg: str, level: str = "info"):
    prefix = {"ok": f"{Colors.GREEN}✓{Colors.END}", "fail": f"{Colors.RED}✗{Colors.END}", 
              "warn": f"{Colors.YELLOW}⚠{Colors.END}", "info": f"{Colors.BLUE}•{Colors.END}"}.get(level, "•")
    print(f"{prefix} {msg}")
...
async def api_post(endpoint: str, payload: Dict, timeout: float = 30.0) -> Dict:
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        return {"status": r.status_code, "data": r.json() if r.headers.get("content-type", "").startswith("application/json") else r.text, "time": r.elapsed.total_seconds()}

async def test_health():
    log("Тест: /health", "info")
    r = await api_post("/health", {}) if False else (await httpx.AsyncClient().get(f"{BASE_URL}/health"))
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{BASE_URL}/health")
    ok = r.status_code == 200 and r.json().get("status") == "ok"
    log(f"/health: {r.status_code} {r.elapsed.total_seconds()*1000:.0f}ms {'✅' if ok else '❌'}", "ok" if ok else "fail")
    return ok
...
async def test_process_basic():
    ...
    log(f"Базовый чат: {tag} | {r['time']*1000:.0f}ms {'✅' if ok else '❌'}", "ok" if ok else "fail")
    return ok
...
async def test_modes():
    ...
    for mode in modes:
        ...
        log(f"  force_mode={mode!r}: {tag} ({model}) {r['time']*1000:.0f}ms {'✅' if ok else '❌'}", "ok" if ok else "fail")
...
async def test_agent_tasks():
    ...
    results = []
    for query, expected_tool in tasks:
        ...
        is_agent = "[🛠️" in tag or "agent" in (r["data"].get("model_used", "") if isinstance(r["data"], dict) else "")
        ok = r["status"] == 200 and is_agent
        results.append((query[:40], ok, tag, reply[:60]))
        log(f"  {query[:40]}: {tag} {'✅' if ok else '❌'}", "ok" if ok else "fail")
    # Очистка тестового файла
    try: os.remove(os.path.expanduser("~/api_test.txt"))
    except: pass
    return all(r[1] for r in results)
...
async def test_privacy_routing():
    ...
    q2 = {"user_id": USER_ID, "text": "мой API ключ sk-1234567890abcdef, что с ним делать?", "force_mode": None}
    ...
    log(f"  С токеном: privacy={pm2} (ожидается CLOUD) {'✅' if pm2=='CLOUD' else '⚠️'}", "ok" if pm2=='CLOUD' else "warn")
    return ok
...
async def test_edge_cases():
    ...
    results = []
    for i, case in enumerate(cases[:4]):  # пропускаем дубликат
        query, desc = case[:2]
        payload = {"user_id": USER_ID, "text": query, "force_mode": None} if len(case) == 2 else case[2]
        try:
            r = await api_post("/process", payload, timeout=60)
            ok = r["status"] in (200, 400, 422)  # 4xx — валидная обработка ошибки
            results.append(ok)
            log(f"  {desc}: {r['status']} {r['time']*1000:.0f}ms {'✅' if ok else '❌'}", "ok" if ok else "fail")
...
async def test_performance():
    ...
    log(f"  Среднее: {avg*1000:.0f}ms, P95: {p95*1000:.0f}ms {'✅' if ok else '⚠️ медленно'}", "ok" if ok else "warn")
    return ok
...
async def test_models_endpoint():
    ...
    log(f"  Локальных: {len(data.get('local', []))}, облачных: {len(data.get('cloud', []))} {'✅' if ok else '❌'}", "ok" if ok else "fail")
    return ok
