"""♻️ Skill Registry: динамический лоадер"""
...
class SkillRegistry:
            ...
            try:
                ...
                for n, o in inspect.getmembers(mod):
                    if callable(o) and hasattr(o, "__skill__"):
                        m = o.__skill__()
                        self.skills[m.get("name", n)] = {"func": o, "desc": m.get("desc", ""), "params": m.get("params", {}), "privacy": m.get("privacy", "CLOUD"), "type": "static"}
            except Exception as e: logging.warning(f"Static load error {f}: {e}")
    ...
    async def load_packs(self):
        ...
        for f in sorted(packs_dir.glob("*.py")):
            ...
            try:
                ...
                for s in getattr(mod, "__skills__", []): self.skills[s["name"]] = s
                logging.info(f"✅ Pack {mod_name}: {len(getattr(mod, '__skills__', []))} loaded")
            except Exception as e: logging.warning(f"⚠️ Pack {mod_name}: {e}")
    ...
    async def load_mcp(self):
        ...
        try:
            ...
            for tname, meta in mcp_client.tools_meta.items():
                if tname in self.skills: continue
                async def wrap(q, ctx, uid, tn=tname, **kw): return await mcp_client.execute(tn, kw)
        ...
        except Exception as e: logging.warning(f"⚠️ MCP load skipped: {e}")
    ...
    def list(self, q=None): return list(self.skills.keys())
