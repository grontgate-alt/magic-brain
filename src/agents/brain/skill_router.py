...
class SkillRouter:
        ...
        for cat in raw.get("categories", []):
            for s in cat.get("skills", []):
                ...
                self.skills[sid] = {
                    ...
                    "name": s.get("name", sid),
                    "desc": s.get("desc", ""),
                    "keywords": s.get("keywords", "").split(","),
                    "privacy": s.get("privacy", "MEDIUM"),
                    "enabled": True,
                    "path": f"agents/skills/{cat.get('id', 'unknown')}/{sid}.py",
                }
    ...
    def sync_to_qdrant(self):
        ...
        if not self.qdrant.collection_exists(COL_NAME):
            self.qdrant.create_collection(
                COL_NAME, vectors_config=qd.VectorParams(size=1024, distance=qd.Distance.COSINE)
            )
        points, ids = [], []
        for sid, m in self.skills.items():
            ...
            points.append(
                qd.PointStruct(
                    id=ids[-1],
                    vector=vec,
                    payload={"skill_id": sid, "name": m["name"], "privacy": m["privacy"]},
                )
            )
    ...
    def search(self, query: str, limit: int = 5, privacy_mode: str = "CLOUD"):
        ...
        for p in hits.points:
            if p.payload["skill_id"] not in self.skills:
                continue
            sk = self.skills[p.payload["skill_id"]]
            if privacy_mode == "LOCAL" and sk["privacy"] == "LOW":
                continue
            if not sk["enabled"]:
                continue
            res.append(
