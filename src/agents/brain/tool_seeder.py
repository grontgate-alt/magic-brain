...
import sys, os, json, logging, importlib, glob

PROJECT_SRC = os.path.expanduser("~/magic-brain-deploy/src")
sys.path.insert(0, PROJECT_SRC)
from agents.brain.tool_db import register_tool, init_db, _get_conn
...
def seed_packs():
    ...
    for pack_path in glob.glob(packs_pattern):
        if os.path.basename(pack_path).startswith("_"): continue
...
def seed_categories():
    ...
    for (domain, name), desc in hints.items():
        conn.execute("INSERT OR REPLACE INTO categories (domain, name, description) VALUES (?, ?, ?)",
                     (domain, name, desc))
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    routes = conn.execute("SELECT DISTINCT domain||'/'||category||'/'||subcategory FROM tools ORDER BY 1").fetchall()
    print(f"📊 В БД: {total} инструментов")
    print("🗂️ Маршруты:")
    for r in routes: print(f"  • {r[0]}")
    conn.close()
