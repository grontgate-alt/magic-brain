...
try:
    ...
    MCP_AVAILABLE = False
    logging.warning("⚠️ MCP library not installed. Run: pip3 install mcp --break-system-packages --user")
                ...
                for tool in result.tools:
                    tools.append({
                        ...
                        "params": {
                            ...
                            for p in (tool.inputSchema.get("properties", {}) if tool.inputSchema else {})
                        },
                        "func": lambda q, ctx, uid, tn=tool.name, **kw: self._execute_tool(name, tn, kw),
                        "privacy": "LOCAL" if name in ["filesystem", "git", "postgres"] else "CLOUD",
                        "source": f"mcp:{name}"
                    })
                return tools
