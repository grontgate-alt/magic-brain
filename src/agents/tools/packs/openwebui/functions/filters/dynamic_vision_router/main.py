...
class Filter:
    ...
    async def inlet(
        ...
        if __user__ is not None and (
            ...
            and not self.valves.enabled_for_admins
            or __user__.get("role") == "user"
            and not self.valves.enabled_for_users
        ):
            return body
        ...
        if not has_images:
            ...
            if user_message_content is not None and isinstance(user_message_content, list):
                has_images = any(item.get("type") == "image_url" for item in user_message_content)
