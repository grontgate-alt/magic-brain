...
async def test():
    ...
    meta = [
        {
            "name": t,
            "desc": registry.skills[t].get("desc", ""),
            "params": registry.skills[t].get("params", {}),
        }
        for t in tools
        if t in registry.skills
    ]
    t_desc = "\n".join(
        [f"- {t['name']}: {t['desc']} (args: {json.dumps(t.get('params', {}))})" for t in meta]
    )
...
If no match: {{"tool": null}}"""
    ...
    try:
        raw = await asyncio.wait_for(
            ollama.chat(model="qwen2.5:3b", prompt=prompt, context=[]), timeout=20
        )
    ...
    try:
        ...
        if not parsed.get("tool"):
            print("   ⚠️ tool is null/empty → router returns None")
        elif parsed.get("conf", 0) < 0.5:
            print(f"   ⚠️ conf={parsed.get('conf')} < 0.5 → router returns None")
        else:
            print(
                f"   🎯 Router would select: {parsed['tool']} with args: {parsed.get('args', {})}"
            )
